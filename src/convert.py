# https://www.kaggle.com/datasets/jubaerad/weapons-in-images-segmented-videos
...
def convert_and_upload_supervisely_project(
    ...
    # project_name = "Weapons in Ima/ges"
    dataset_root = (
        "/home/grokhi/rawdata/weapons-in-images"
    )
    batch_size = 30
    images_ext = ".jpg"
    ann_ext = ".txt"
    ...
    obj_class = sly.ObjClass("weapon", sly.Rectangle)
    project = api.project.create(workspace_id, project_name, change_name_if_conflict=True)
    meta = sly.ProjectMeta(obj_classes=[obj_class])
    api.project.update_meta(project.id, meta.to_json())

    for ds_name in ["train-weapons_in_images", "test-cctv"]:

        dataset_path = os.path.join(dataset_root, "Weapons-in-Images/Weapons-in-Images") if ds_name=="train-weapons_in_images" else os.path.join(dataset_root, "Weapon in Images (Segmented Video)/All")
        ...
        for images_names_batch in sly.batched(images_names, batch_size=batch_size):
            ...
            if ds_name=="train-weapons_in_images":
